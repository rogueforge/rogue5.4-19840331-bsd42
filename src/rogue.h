...
char	*charge_str(), *choose_str(), *inv_name(), *nullstr(),
	*num(), *ring_num(), *set_mname(), *vowelstr(),
	*unctrl();

int	accnt_maze(), badcheck(), bounce(), call(),
	check_level(), conn(), current(), d_level(), dig(),
	discard(), do_chase(), do_maze(), do_motion(), do_pot(),
	doadd(), door(), drain(), draw_room(), encwrite(),
	erase_lamp(), fall(), fire_bolt(), help(), hit(),
	horiz(), identify(), illcom(), invis_on(), killed(),
	miss(), money(), move_monst(), move_msg(), nameit(),
	numpass(), passnum(), pr_spec(), put_bool(),
	put_inv_t(), put_str(), put_things(), putpass(),
	raise_level(), remove_mon(), reset_last(), rust_armor(),
	save_file(), search(), set_know(), set_oldch(),
	strucpy(), thunk(), treas_room(), turnref(), u_level(),
	uncurse(), vert(), wait_for(), waste_time(), get_bool(),
	get_inv_t(), get_num(), get_sf(), get_str(), trip_ch(),
	chase(), diag_ok(), fallpos(), find_floor(), is_magic(),
	levit_check(), roll_em(), see_monst(), seen_stairs(),
	turn_ok(), turn_see(), be_trapped(), floor_ch(),
	pack_char();

bool	dropcheck(), pack_room();
